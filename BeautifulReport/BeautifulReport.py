...
class ReportTestResult(unittest.TestResult):
    ...
    def get_testcase_property(test) -> tuple:
        """
        接受一个test, 并返回一个test的class_name, method_name, method_doc, author属性
        :param test:
        :return: (class_name, method_name, method_doc, author) -> tuple
        """
        class_name = test.__class__.__qualname__
        method_name = test.__dict__['_testMethodName']
        method_doc = test.__dict__['_testMethodDoc']
        author = test.__class__.__author__
        return class_name, method_name, method_doc, author
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def output_report(self, theme):
        ...
        with open(override_path.replace('\\', '/') + self.filename, 'w', encoding='utf-8', newline='\n') as write_file:
